		...
		<>
			<div
				...
				onClick={() => {
					...
					sidebarRef.current?.classList.remove("-translate-x-[5rem]");
			...
			<aside
				ref={sidebarRef}
				className={`pt-[64px] w-screen h-screen absolute top-0 left-0 p-4 transition -translate-x-full bg-black/25 dark:bg-black/75 md:dark:bg-transparent md:translate-x-0 md:relative z-40 md:w-1/2 md:h-auto md:pr-8 md:block md:z-0 md:bg-transparent`}
			>
					...
					<button
						...
						onClick={() => {
							if (sidebarRef.current?.classList.contains("-translate-x-full")) {
								...
								sidebarRef.current?.classList.add("-translate-x-[5rem]");
