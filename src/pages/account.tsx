...
import { API_SUCCESS, BASE_URL, ShortLinkType } from "../utils/constants";
    ...
    if (session && session.user && session.user.email) {
        try {
            const cursor = db
                ...
                .find({ email: { $eq: session.user.email.toString() } });
            const entries = (await cursor.toArray()).map(
                ({ _id, ...entry }) => entry
            ) as ShortLinkType[];
...
const Account = ({
    ...
    const linkDeleteHandler = async (slug: string) => {
        const response = await (
            await fetch(`/api/delete-url?slug=${slug}`)
        ).json();
		...
		if (!response.error) {
			...
			setSlugDeleteAPiMessage(response.error);
		}
                                ...
                                {linksData.map((item) => (
                                    <tr key={item.slug} className="h-8">
                                        ...
                                        <td className="aspect-square p-2 w-16 h-full relative flex justify-end items-center">
                                            ...
                                            <YesAndNoPopup
                                                title={`Delete "${
                                                    item.userLink.slice(0, 40) +
                                                    "..."
                                                }" ?`}
                                                isOpen={showDeleteModal}
                                                onClose={onDeletePopupClose}
                                                onYes={() =>
                                                    onDeletePopupYes(item.slug)
                                                }
